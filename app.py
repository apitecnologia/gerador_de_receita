### ARQUIVO: app.py (COMANDO DE EXCLUSÃO ADICIONADO) ###
...
# (Todas as suas rotas @app.route permanecem exatamente as mesmas)
...
# --- NOVO COMANDO ADICIONADO ABAIXO ---
...
def delete_user_command():
    ...
    if user:
        if user.is_admin:
            print(f"Erro: Não é permitido excluir um usuário administrador ('{username}') por este comando.")
            return
        
        confirm = input(f"Tem certeza que deseja excluir o usuário '{username}'? Todos os seus dados serão perdidos. (s/n): ")
        if confirm.lower() == 's':
            db.session.delete(user)
            db.session.commit()
